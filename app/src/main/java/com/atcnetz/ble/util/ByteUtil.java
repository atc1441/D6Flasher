...
public class ByteUtil {
    ...
    public static byte[] intToByte(int i, int len) {
        ...
        abyte[3] = (byte) ((ViewCompat.MEASURED_STATE_MASK & i) >> 24);
        return abyte;
    }
    ...
    public static int bytesToInt(byte[] bytes) {
        ...
        if (bytes.length == 4) {
            return (((bytes[0] & 255) | ((bytes[1] << 8) & 65280)) | ((bytes[2] << 16) & 16711680)) | ((bytes[3] << 24) & ViewCompat.MEASURED_STATE_MASK);
        }
        if (bytes.length == 2) {
    ...
    public static int bytesToIntForVersion(byte[] bytes) {
        ...
        if (bytes.length == 4) {
            return (((bytes[3] & 255) | ((bytes[2] << 8) & 65280)) | ((bytes[1] << 16) & 16711680)) | ((bytes[0] << 24) & ViewCompat.MEASURED_STATE_MASK);
        }
        if (bytes.length == 2) {
            return (bytes[0] & 255) | ((bytes[1] << 8) & 65280);
        }
